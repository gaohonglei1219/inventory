...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="parameter/listParameters.do" method="post" name="parameterForm" id="parameterForm">
			<table>
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="selectData" value="${selectData }" placeholder="这里输入关键词" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td><input class="span10 date-picker" name="lastUpdateDateStart" id="lastUpdateDateStart" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="开始日期" title="最近修改开始"/></td>
					<td><input class="span10 date-picker" name="lastUpdateDateEnd"   id="lastUpdateDateEnd"   type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="最近修改结束"/></td>
					
					<c:if test="${QX.cha == 1 }">
					<td style="vertical-align:top;"><button class="btn btn-mini btn-light" onclick="search();" title="检索"><i id="nav-search-icon" class="icon-search"></i></button></td>
					<td style="vertical-align:top;"><a class="btn btn-mini btn-light" onclick="window.location.href='<%=basePath%>parameter/listTab.do';" title="切换模式"><i id="nav-search-icon" class="icon-exchange"></i></a></td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		$(function() {
			...
			//日期框
			$('#lastUpdateDateStart').datepicker({ 
			todayBtn : "linked", 
			autoclose : true, 
			todayHighlight : true, 
			endDate : new Date() 
			...
			$('#lastUpdateDateEnd').datepicker('setStartDate',startTime); 
			}); 
			//结束时间： 
			$('#lastUpdateDateEnd').datepicker({ 
			todayBtn : "linked", 
			autoclose : true, 
			todayHighlight : true, 
			endDate : new Date() 
			...
			var endTime = e.date; 
			$('#lastUpdateDateStart').datepicker('setEndDate',endTime); 
			}); 
