...
<script type="text/javascript">
	$(top.hangge());	
	 $(function() {
		 ...
		 input.blur(function() {
			 ...
			 {
				 if (newtxt != txt) 
				 $.post("<%=basePath%>newMenu/saveEditParentMenuName.do",{"menuId":a,
					 "menuName":newtxt
				 });
				 td.html(newtxt);
			 }else{
				 if (newtxt != txt)$.post("<%=basePath%>newMenu/saveEditParentMenuOrder.do",{"menuId":a,
					 "menuOrder":newtxt
				 });
				 td.html(newtxt);
			 }
	...
	function addmenu(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增菜单";
		 diag.URL = '<%=basePath%>newMenu/toAdd.do';
		 diag.Width = 223;
		 diag.Height = 256;
	...
	function editmenu(menuId){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑菜单";
		 diag.URL = '<%=basePath%>newMenu/toEdit.do?menuId='+menuId;
		 diag.Width = 223;
		 diag.Height = 256;
		 diag.CancelEvent = function(){ //关闭事件
	...
	function editTb(menuId){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑图标";
		 diag.URL = '<%=basePath%>newMenu/toEditIcon.do?menuId='+menuId;
		 diag.Width = 530;
		 diag.Height = 150;
		 diag.CancelEvent = function(){ //关闭事件
			if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function delmenu(menuId,isParent){
		...
		if(flag){
			top.jzts();
			var url = "<%=basePath%>newMenu/saveDelete.do?menuId="+menuId;			
...
<body>
<form action="newMenu/toEdit.do" name="menuForm" id="menuForm" method="post">
